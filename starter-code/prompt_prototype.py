...
def _fallback_response(user_input: str) -> str:
    """Deterministic fallback used when API key is unavailable or Gemini quota is exhausted.
    This keeps the prototype runnable in offline or quota-limited environments.
    """
    lowered = user_input.lower()
    if "2%" in lowered or "pin hiện tại báo 2%" in lowered:
        return '{"action": "dispatch_mobile_charger", "reason": "Battery level under critical threshold of 5%. Cannot reach station safely."}'
    return "[DRAFT_ONLY] Tôi đã nhận được yêu cầu và đang chờ phê duyệt từ điều phối viên."
